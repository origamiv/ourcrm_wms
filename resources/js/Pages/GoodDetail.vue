...
const titleIsHardcoded = computed(() => !good.value?.name && !good.value?.shortname);
...
const otherPropertiesAreHardcoded = computed(() => !flattenProperties(good.value?.src).length);
    ...
    <div class="good-detail-page">
        ...
        <div class="good-detail-heading">
            <div>
                ...
                <h1 :class="{ 'hardcoded-value': titleIsHardcoded }">{{ title }}</h1>
        ...
        <main class="good-detail-layout"><div class="good-detail-sections">
            ...
            <section class="good-detail-section" :class="{ open: expanded.stock }"><button class="good-section-toggle" type="button" @click="toggle('stock')"><span>Склад</span><small class="good-section-summary hardcoded-value">{{ stockSummary }}</small><b>{{ expanded.stock ? '⌃' : '⌄' }}</b></button><div v-if="expanded.stock" class="good-section-content"><div class="good-stock-filters"><label>Склад<select v-model="warehouseFilter"><option value="all">Все склады</option><option v-for="item in [...new Set(demoStock.map((row) => row.warehouse))]" :key="item" :value="item">{{ item }}</option></select></label><label>Ячейка<select v-model="cellFilter"><option value="all">Все ячейки</option><option v-for="item in [...new Set(demoStock.map((row) => row.cell))]" :key="item" :value="item">{{ item }}</option></select></label><label>Клиент<select v-model="clientFilter"><option value="all">Все клиенты</option><option v-for="item in [...new Set(demoStock.map((row) => row.client))]" :key="item" :value="item">{{ item }}</option></select></label></div><div class="good-mini-table"><div class="good-mini-row header"><span>Склад / ячейка</span><span>Клиент</span><span>Остаток</span></div><div v-for="item in filteredStock" :key="item.cell" class="good-mini-row hardcoded-value"><span><strong>{{ item.warehouse }}</strong><small>{{ item.cell }}</small></span><span>{{ item.client }}</span><b>{{ item.quantity }} шт.</b></div><p v-if="!filteredStock.length" class="empty-state">По выбранным фильтрам остатков нет.</p></div></div></section>
            <section class="good-detail-section" :class="{ open: expanded.orders }"><button class="good-section-toggle" type="button" @click="toggle('orders')"><span>Заказы с этим товаром</span><small class="good-section-summary hardcoded-value">{{ ordersSummary }}</small><b>{{ expanded.orders ? '⌃' : '⌄' }}</b></button><div v-if="expanded.orders" class="good-section-content"><div class="good-mini-table"><div class="good-mini-row header"><span>Заказ</span><span>Клиент / статус</span><span>Количество</span></div><div v-for="item in demoOrders" :key="item.number" class="good-mini-row hardcoded-value"><span><strong>{{ item.number }}</strong><small>{{ item.date }}</small></span><span>{{ item.client }}<small>{{ item.status }}</small></span><b>{{ item.quantity }} шт.</b></div></div></div></section>
            <section class="good-detail-section" :class="{ open: expanded.services }"><button class="good-section-toggle" type="button" @click="toggle('services')"><span>ТЗ и услуги</span><small class="good-section-summary hardcoded-value"><em class="good-summary-badge">ТЗ заполнено</em> · 23 услуги</small><b>{{ expanded.services ? '⌃' : '⌄' }}</b></button><div v-if="expanded.services" class="good-detail-hardcoded good-section-content"><div class="good-service-list"><div v-for="item in demoServices" :key="item.name"><strong>{{ item.name }}</strong><span>{{ item.status }}</span><b>{{ item.price }}</b></div></div></div></section>
