...
interface Task { key: string; label: string; type: string; options?: { required?: Record<string, any>; optional?: Record<string, any> } }
...
function moscowDate(): string { const parts = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Moscow", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date()); const get = (type: string) => parts.find((part) => part.type === type)?.value ?? ""; return `${get("year")}-${get("month")}-${get("day")}`; }
function defaultSchedule(): Schedule { return { kind: "interval", interval: 1, unit: "hour", week_interval: 1, weekdays: [1], month_days: [1], start_date: moscowDate(), end_date: "", time: "09:00" }; }
function tenantValue(): string { return document.querySelector<HTMLMetaElement>('meta[name="wms-tenant"]')?.content ?? ""; }
function taskParams(taskKey: string): Record<string, any> { const task = tasks.value.find((item) => item.key === taskKey); const result: Record<string, any> = { arguments: {}, options: {} }; for (const [key, spec] of Object.entries(task?.options?.required ?? {})) result.arguments[key] = spec.default === "current_tenant" ? tenantValue() : (spec.default ?? null); for (const [key, spec] of Object.entries(task?.options?.optional ?? {})) (task?.type === "job" ? result.arguments : result.options)[key] = spec.default === "current_tenant" ? tenantValue() : (spec.default ?? (spec.type === "boolean" ? false : "")); return result; }
function reset(row?: Row) { editing.value = row ?? null; editorOpen.value = true; form.value = row ? { name: row.name, task_key: row.task_key, params: JSON.stringify(row.params ?? {}, null, 2), status: row.status, schedule: { ...defaultSchedule(), ...row.schedule, weekdays: [...(row.schedule.weekdays ?? [])], month_days: [...(row.schedule.month_days ?? [])] } } : { name: "", task_key: tasks.value[0]?.key ?? "", params: JSON.stringify(taskParams(tasks.value[0]?.key ?? ""), null, 2), status: 1, schedule: defaultSchedule() }; repeatPreset.value = row ? presetFor(row.schedule) : "hour_1"; }
function applyTaskDefaults() { if (!editing.value) form.value.params = JSON.stringify(taskParams(form.value.task_key), null, 2); }
...
<template>
    ...
    <TableViewLayout :editor-open="editorOpen">
        <section class="users-list scheduler-main">
            ...
            <div class="table-scroll"><table><thead><tr><th>#</th><th>Название</th><th>Задача</th><th>Расписание</th><th>Статус</th><th>Следующий запуск</th><th>Последний запуск</th><th>Действия</th></tr></thead>
                <tbody><tr v-if="loading"><td colspan="8" class="empty-cell">Загрузка…</td></tr><tr v-else-if="!rows.length"><td colspan="8" class="empty-cell">Расписания ещё не созданы</td></tr>
                    <tr v-for="row in rows" :key="row.id"><td><span class="row-id">{{ row.id }}</span></td><td><strong>{{ row.name }}</strong></td><td>{{ row.task_label }}</td><td>{{ row.schedule_label }}</td><td><button class="status-badge" :class="row.status ? 'status-completed' : 'status-failed'" type="button" @click="reset(row)">{{ row.status ? 'Включено' : 'Выключено' }}</button></td><td>{{ format(row.next_run_at) }}</td><td>{{ format(row.last_run_at) }}</td><td><div class="row-actions"><button type="button" aria-label="История запусков" title="История запусков" @click="showHistory(row)"><img src="/design/crm/view.svg" alt=""></button><button type="button" aria-label="Редактировать" title="Редактировать" @click="reset(row)"><img src="/design/crm/edit.svg" alt=""></button><button type="button" aria-label="Удалить" title="Удалить" @click="remove(row)"><img src="/design/crm/delete.svg" alt=""></button></div></td></tr>
                </tbody></table></div>
        </section>
        <aside v-if="editorOpen" class="editor scheduler-editor" aria-label="Расписание">
            ...
            <div class="editor-content"><p v-if="error" class="notice error" role="alert">{{ error }}</p><form @submit.prevent="save"><label>Название<input v-model="form.name" required type="text"></label><label>Задача<select v-model="form.task_key" @change="applyTaskDefaults"><option v-for="task in tasks" :key="task.key" :value="task.key">{{ task.label }}</option></select></label><label>Параметры JSON<textarea v-model="form.params" rows="5"></textarea></label><label class="check"><input v-model="form.status" type="checkbox" :true-value="1" :false-value="0"> Расписание включено</label>
                    ...
                    <div class="form-grid repeat-dates"><label>Начало<RussianDateInput v-model="form.schedule.start_date" /></label><label>Время<input v-model="form.schedule.time" type="time"></label><label>До (необязательно)<RussianDateInput v-model="form.schedule.end_date" /></label></div>
