...
const today = new Date(),
    ...
    report = ref<any>({ days: [], rows: [] }),
    loading = ref(false),
    error = ref(""),
    calendarRef = ref<HTMLElement | null>(null);
const advancedFilters = useFilterPresets("worktime");
const advancedFields: FilterField[] = [
    { id: "user_id", label: "ID сотрудника", type: "number" },
    { id: "employee", label: "Сотрудник", type: "text" },
...
const employeeName = (user: any) =>
    [user.last_name, user.name].filter(Boolean).join(" ") ||
    user.email ||
    `№${user.id}`;
const employeeInitials = (user: any) =>
    [user.last_name, user.name]
        .filter(Boolean)
        .map((value: string) => value.trim().slice(0, 1))
        .join("")
        .slice(0, 2)
        .toUpperCase() || "?";
const employeeImage = (user: any) =>
    user.avatar_url ||
    user.avatar ||
    user.photo_url ||
    user.photo ||
...
const hours = (minutes: number) =>
    minutes ? `${Math.ceil(minutes / 60)}ч` : "";
const label = (day: string) => {
    const d = isoDate(day)!;
    return `${String(d.getDate()).padStart(2, "0")} ${weekdays[d.getDay()]}`;
};
const isWeekend = (day: string) => {
    const index = isoDate(day)!.getDay();
    return index === 0 || index === 6;
};
const demoAbsenceState = (day: string) => {
    const pastWeekdays = report.value.days.filter(
        (item: string) =>
            !isWeekend(item) && item < new Date().toISOString().slice(0, 10),
    );
    const index = pastWeekdays.indexOf(day);
    return index === 0 ? "vacation" : index === 1 ? "sick" : null;
};
const stateFor = (row: any, day: string) => {
    const value = info(row, day);
    const demoAbsence = demoAbsenceState(day);
    if (demoAbsence && value.state === "empty") return demoAbsence;
    const minutes = Number(value.worked_minutes || 0);
    if (minutes > 480) return "overtime";
    if (minutes >= 480) return "full";
    if (minutes > 0 && minutes < 240) return "short";
...
const segments = (row: any) => {
    const result: any[] = [];
    report.value.days.forEach((day: string, index: number) => {
        const state = stateFor(row, day);
        const mergeable = ["vacation", "sick"].includes(state);
...
const monthGroups = computed(() => {
    ...
    for (const day of report.value.days) {
...
async function scrollToToday() {
    ...
    if (
        !calendar ||
        !report.value.days.length ||
        !window.matchMedia("(max-width: 900px)").matches
    )
        return;
    const currentDay = new Date().toISOString().slice(0, 10);
    const target =
        calendar.querySelector<HTMLElement>(`[data-day="${currentDay}"]`) ||
        calendar.querySelector<HTMLElement>("[data-day]");
    if (!target) return;
    calendar.scrollLeft = Math.max(
        0,
        target.offsetLeft - (calendar.clientWidth - target.offsetWidth) / 2,
    );
}
async function load() {
    ...
    loading.value = true;
    try {
        report.value = await http(
            `/web/worktime?from=${from.value}&to=${to.value}`,
        );
        await scrollToToday();
    } catch (e: any) {
        error.value = e?.message || "Не удалось загрузить график.";
    } finally {
        loading.value = false;
    }
}
...
<template>
    <div class="users-workspace worktime-page">
        <section class="users-list">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        >
                            <td class="employee">
                                <span
                                    ...
                                    :title="employeeName(row.user)"
                                >
                                    <img
                                        v-if="employeeImage(row.user)"
                                        :src="employeeImage(row.user)"
                                        alt=""
                                    />
                                    <span v-else>{{
                                        employeeInitials(row.user)
                                    }}</span>
                                </span>
                            ...
                            <td
                                ...
                                :class="[
                                    ...
                                    segment.state,
                                    {
                                        weekend:
                                            segment.days === 1 &&
                                            isWeekend(segment.day),
                                    },
                                ]"
                            >
                                {{
                                    segment.state === "vacation"
                                        ? "Отпуск"
                                        : segment.state === "sick"
                                          ? "Больничный"
                                          : segment.days === 1
                                            ? hours(
                                                  info(row, segment.day)
                                                      .worked_minutes,
                                              )
                                            : ""
                                }}
                        ...
                        <tr v-if="!filteredRows.length">
                            <td :colspan="report.days.length + 2" class="empty">
                                Нет данных
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="legend">
                <span><i class="overtime"></i>Переработка</span
                ><span><i class="full"></i>8 часов</span
                ><span><i class="partial"></i>Неполный день</span
                ><span><i class="short"></i>Менее 4 часов</span
                ><span><i class="leave"></i>Отпуск / больничный</span>
            </div>
        </section>
    </div>
</template>
...
@media (max-width: 900px) {
    ...
    .worktime-page :deep(.ribbon-group) {
        width: 100%;
        margin-bottom: 0;
    }
    .worktime-page :deep(.module-tabs) {
        width: 100%;
        max-width: 100%;
    }
    .worktime-heading {
    ...
    .calendar .employee-avatar {
        ...
        border-radius: 50%;
        overflow: hidden;
        background: linear-gradient(135deg, #1e892f, #66c98a);
        color: #fff;
        font-size: 12px;
        font-weight: 700;
    }
