...
<template>
    <div class="users-workspace worktime-page">
        <section class="users-list">
            ...
            <div class="table-scroll calendar" :class="{ busy: loading }">
                <table>
                    <thead>
                        <tr class="month-row">
                        ...
                        <tr class="day-row">
                            <th v-for="day in report.days" :key="day">{{ label(day) }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="row in report.rows" :key="row.user.id">
                            <td class="employee"><strong>{{ [row.user.last_name, row.user.name].filter(Boolean).join(' ') || row.user.email }}</strong></td>
                            <td v-for="segment in segments(row)" :key="segment.day" :colspan="segment.days" :class="['day', segment.state, { weekend: segment.days === 1 && isWeekend(segment.day) }]">{{ segment.state === 'vacation' ? 'Отпуск' : segment.state === 'sick' ? 'Больничный' : segment.days === 1 ? hours(info(row, segment.day).worked_minutes) : '' }}</td>
