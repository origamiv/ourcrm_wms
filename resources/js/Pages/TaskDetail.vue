    ...
    <div class="taskDetailPage">
        ...
        <main v-else class="taskDetailLayout">
            <div class="taskDetailSections">
                <section class="taskDetailSection" :class="{ taskDetailOpen: expanded.general }"><button class="taskDetailToggle" type="button" @click="toggle('general')"><span>Общее инфо</span><small>{{ productCount }} / {{ pieceCount }} · {{ progress }}%</small><b>{{ expanded.general ? '⌃' : '⌄' }}</b></button><div v-if="expanded.general" class="taskDetailContent"><div class="taskDetailStatus"><span>Статус задачи</span><strong>{{ value(status?.name) }}</strong><label>Перевести в<select :value="task.status_id" :disabled="busy" @change="changeStatus(($event.target as HTMLSelectElement).value)"><option v-for="option in statusOptions" :key="option.id" :value="option.id">{{ option.name }}</option></select></label></div><div class="taskDetailGrid"><div><span>Выполнена</span><strong>{{ date(task.completed_at, 'Еще не выполнена') }}</strong></div><div><span>Подтверждение завершения</span><strong>{{ task.confirmed_at ? `Подтверждено ${date(task.confirmed_at)}` : 'Не подтверждено' }}</strong></div><div><span>Товаров / штук</span><strong>{{ productCount }} / {{ pieceCount }}</strong></div></div></div></section>
                <div class="taskDetailGeneralExtras"><div><span>ТЗ</span><p>{{ technicalBrief }}</p></div><div><span>Комментарий</span><p>{{ value(task.comment) }}</p></div><div><span>Внутренний комментарий</span><p>{{ value(task.internal_comment) }}</p></div><div><span>Файлы</span><input ref="fileInput" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.jpg,.jpeg,.png" @change="uploadFile" /><button type="button" :disabled="busy" @click="fileInput?.click()">Загрузить файл</button><small v-if="taskFiles.length">Загружено файлов: {{ taskFiles.length }}</small></div></div>
