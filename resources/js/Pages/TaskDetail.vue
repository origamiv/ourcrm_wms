                ...
                <section class="taskDetailSection" :class="{ taskDetailOpen: expanded.general }">
                    ...
                    <div v-if="expanded.general" class="taskDetailContent">
                        ...
                        <div class="taskDetailGeneralExtras">
                            ...
                            <div class="taskDetailGeneralExtra"><span>Файлы</span><input ref="fileInput" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.jpg,.jpeg,.png" @change="uploadFile" /><button type="button" :disabled="busy" @click="fileInput?.click()">Загрузить файл</button><small v-if="taskFiles.length">Загружено файлов: {{ taskFiles.length }}</small></div>
                        </div>
                    </div>
                </section>
