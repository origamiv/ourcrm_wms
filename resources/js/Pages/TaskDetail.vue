        ...
        <main v-else class="taskDetailLayout">
            <div class="taskDetailSections">
                ...
                <section class="taskDetailSection" :class="{ taskDetailOpen: expanded.general }">
                    <button class="taskDetailToggle" type="button" @click="toggle('general')"><span>Общее инфо</span><strong class="taskDetailHeaderStatus">{{ value(status?.name) }}</strong><small>{{ productCount }} / {{ pieceCount }} · {{ progress }}%</small><b>{{ expanded.general ? '⌃' : '⌄' }}</b></button>
                    <div v-if="expanded.general" class="taskDetailContent">
                        <div class="taskDetailGrid"><div><span>Подтверждение завершения</span><strong>{{ task.confirmed_at ? `Подтверждено ${date(task.confirmed_at)}` : 'Не подтверждено' }}</strong></div><div><span>Товаров / штук</span><strong>{{ productCount }} / {{ pieceCount }}</strong></div><div><label class="taskDetailStatusLabel">Перевести в<select :value="task.status_id" :disabled="busy" @change="changeStatus(($event.target as HTMLSelectElement).value)"><option v-for="option in statusOptions" :key="option.id" :value="option.id">{{ option.name }}</option></select></label></div></div>
                        <div class="taskDetailGeneralExtras">
                            ...
                            <div class="taskDetailGeneralExtra taskDetailInternalComment"><span>Внутренний комментарий</span><p>{{ value(task.internal_comment) }}</p></div>
