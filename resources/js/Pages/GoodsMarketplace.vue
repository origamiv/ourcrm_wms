...
const matchLabels: Record<string, string> = { auto: "Автоматически", created: "Создан", manual: "Вручную", failed: "Неуспешно", error: "Ошибка сопоставления", unmatched: "Не сопоставлено" };
...
function matchClass(row: MarketplaceRow) { const type = String(row.match_type ?? ""); if (type === "created") return "match-created"; if (["failed", "error", "unmatched"].includes(type)) return "match-failed"; return ""; }
