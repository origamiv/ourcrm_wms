...
<template>
    ...
    <section class="marketplace-catalog users-workspace">
        ...
        <div class="catalog-filters">
            ...
            <details class="filter-menu"><summary>Интеграции<span v-if="integrationFilter.length">: {{ integrationFilter.length }}</span></summary><div class="filter-options"><label v-for="item in integrations" :key="item"><input v-model="integrationFilter" type="checkbox" :value="item" />{{ webhookNameById(item) }}</label></div></details>
            <details class="filter-menu"><summary>Маркетплейсы<span v-if="marketplaceFilter.length">: {{ marketplaceFilter.length }}</span></summary><div class="filter-options"><label v-for="item in marketplaces" :key="item"><input v-model="marketplaceFilter" type="checkbox" :value="String(item)" />{{ label({ marketplace: item } as MarketplaceRow) }}</label></div></details>
            <details class="filter-menu"><summary>Сопоставления<span v-if="matchFilter.length">: {{ matchFilter.length }}</span></summary><div class="filter-options"><label v-for="item in matchOptions" :key="item.value"><input v-model="matchFilter" type="checkbox" :value="item.value" />{{ item.label }}</label></div></details>
        </div>
        <div v-if="integrationFilter.length || marketplaceFilter.length || matchFilter.length" class="filter-chips"><button v-for="item in integrationFilter" :key="`integration-${item}`" type="button" class="filter-chip" @click="removeFilter(integrationFilter, item)">{{ webhookNameById(item) }} ×</button><button v-for="item in marketplaceFilter" :key="`marketplace-${item}`" type="button" class="filter-chip" @click="removeFilter(marketplaceFilter, item)">{{ label({ marketplace: item } as MarketplaceRow) }} ×</button><button v-for="item in matchFilter" :key="`match-${item}`" type="button" class="filter-chip" @click="removeFilter(matchFilter, item)">{{ matchLabels[item] }} ×</button></div>
        <div v-if="marketplaceStore.error.value" class="catalog-message">{{ marketplaceStore.error.value }}</div>
        <div class="catalog-table-wrap table-scroll"><table><colgroup><col class="col-id" /><col class="col-marketplace" /><col class="col-integration" /><col class="col-product" /><col class="col-sku" /><col class="col-barcode" /><col class="col-good" /><col class="col-match" /><col class="col-sync" /></colgroup><thead><tr><th>#</th><th>МП</th><th>Интеграция</th><th>Товар маркетплейса</th><th>Внешний SKU</th><th>Штрихкоды</th><th>Мастер-каталог WMS</th><th>Сопоставление</th><th>Синхронизация</th></tr></thead><tbody><tr v-for="row in visible" :key="row.id"><td class="id-column">{{ row.id }}</td><td><span class="marketplace-badge" :class="`mp-${row.marketplace}`">{{ label(row) }}</span></td><td>{{ webhookName(row) }}</td><td><div class="product-name">{{ row.name || "Без названия" }}</div><small>ID: {{ row.external_id || "—" }}</small></td><td>{{ row.offer_id || row.external_sku || "—" }}</td><td>{{ (row.barcodes ?? []).join(", ") || "—" }}</td><td><a v-if="row.good_id" class="good-link" :href="`/goods/goods/${row.good_id}/view`">#{{ row.good_id }} · {{ goodName(row) }}</a><span v-else class="muted">Не сопоставлен</span></td><td><span class="match-badge" :class="matchClass(row)">{{ matchLabel(row) }}</span></td><td>{{ formatDate(row.synced_at) }}</td></tr><tr v-if="!visible.length"><td colspan="9" class="catalog-message">{{ marketplaceStore.ready.value ? "По выбранным условиям товары не найдены." : "Загрузка каталога…" }}</td></tr></tbody></table></div>
...
.filter-menu { position: relative; min-width: 170px; } .filter-menu summary { list-style: none; cursor: pointer; border: 1px solid #d5e3d8; border-radius: 7px; padding: 10px 12px; background: #fff; color: #1c3024; } .filter-menu summary::-webkit-details-marker { display: none; } .filter-menu summary::after { content: "▾"; float: right; color: #687a70; } .filter-menu[open] summary { border-color: #1e892f; } .filter-options { position: absolute; z-index: 10; top: calc(100% + 5px); left: 0; min-width: 100%; max-height: 280px; overflow-y: auto; padding: 7px; border: 1px solid #d5e3d8; border-radius: 7px; background: #fff; box-shadow: 0 8px 20px rgb(18 51 34 / 12%); } .filter-options label { display: flex; align-items: flex-start; gap: 7px; padding: 7px 5px; color: #1c3024; white-space: nowrap; cursor: pointer; } .filter-options input { margin-top: 2px; min-width: 15px; }
.filter-chips { display: flex; flex-wrap: wrap; gap: 7px; margin-bottom: 18px; } .filter-chip { border: 0; border-radius: 999px; padding: 6px 10px; background: #e4f4e7; color: #247435; cursor: pointer; font-size: 12px; }
