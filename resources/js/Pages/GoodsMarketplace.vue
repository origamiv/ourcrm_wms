...
<template>
    ...
    <section class="marketplace-catalog users-workspace">
        ...
        <div class="catalog-table-wrap table-scroll"><table><colgroup><col class="col-id" /><col class="col-marketplace" /><col class="col-integration" /><col class="col-product" /><col class="col-sku" /><col class="col-barcode" /><col class="col-good" /><col class="col-match" /><col class="col-sync" /></colgroup><thead><tr><th>#</th><th>МП</th><th>Интеграция</th><th>Товар маркетплейса</th><th>Внешний SKU</th><th>Штрихкоды</th><th>Мастер-каталог WMS</th><th>Сопоставление</th><th>Синхронизация</th></tr></thead><tbody><tr v-for="row in visible" :key="row.id"><td class="id-column">{{ row.id }}</td><td><span class="marketplace-badge" :class="`mp-${row.marketplace}`">{{ label(row) }}</span></td><td>{{ webhookName(row) }}</td><td><div class="product-name">{{ row.name || "Без названия" }}</div><small>ID: {{ row.external_id || "—" }}</small></td><td>{{ row.offer_id || row.external_sku || "—" }}</td><td>{{ (row.barcodes ?? []).join(", ") || "—" }}</td><td><div class="good-cell"><button type="button" class="edit-good" aria-label="Изменить мастер-товар" title="Изменить мастер-товар" @click="startMatch(row)">✎</button><a v-if="row.good_id" class="good-link" :href="`/goods/goods/${row.good_id}/view`">#{{ row.good_id }} · {{ goodName(row) }}</a><span v-else class="muted">Не сопоставлен</span><div v-if="editingId === String(row.id)" class="good-editor"><div class="good-search-row"><input v-model="codeSearch" type="search" placeholder="Код" @focus="goodListOpen = true" @input="goodListOpen = true" @keydown="moveGood" /><input v-model="nameSearch" type="search" placeholder="Наименование" @focus="goodListOpen = true" @input="goodListOpen = true" @keydown="moveGood" /><button type="button" class="good-save" :disabled="!selectedGood || Number(selectedGood.is_category) === 1 || savingMatch" @click="saveMatch(row)">{{ savingMatch ? "Сохранение…" : "Сохранить" }}</button><button type="button" class="good-cancel" @click="closeMatch">×</button></div><div v-if="goodListOpen" class="good-options" role="listbox" aria-label="Товары мастер-каталога"><button v-for="(good, index) in goodOptions" :key="String(good.id)" type="button" class="good-option" :class="{ active: index === activeGoodIndex, category: Number(good.is_category) === 1 }" :disabled="Number(good.is_category) === 1" role="option" :aria-selected="selectedGood?.id === good.id" @click="chooseGood(good)"><span>#{{ good.id }}</span><span>{{ good.code || "—" }}</span><strong>{{ good.name || "Без названия" }}</strong><em v-if="Number(good.is_category) === 1">Категория</em></button><div v-if="!goodOptions.length" class="good-options-empty">Товары не найдены.</div></div><div v-if="matchError" class="good-error">{{ matchError }}</div></div></div></td><td><span class="match-badge" :class="matchClass(row)">{{ matchLabel(row) }}</span></td><td>{{ formatDate(row.synced_at) }}</td></tr><tr v-if="!visible.length"><td colspan="9" class="catalog-message">{{ marketplaceStore.ready.value ? "По выбранным условиям товары не найдены." : "Загрузка каталога…" }}</td></tr></tbody></table></div>
