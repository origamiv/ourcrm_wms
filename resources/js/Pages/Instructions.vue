...
const section = computed(() => new URLSearchParams(page.url.split("?")[1] || "").get("section") || "");
...
function chooseFile(event: Event) { form.value.file = (event.target as HTMLInputElement).files?.[0] ?? null; }
async function save() {
...
.instruction-editor { margin-bottom: 24px; padding: 18px; border: 1px solid #dcecef; border-radius: 10px; background: #f8fbfa; }.instruction-editor h1 { font-size: 18px; margin-bottom: 16px; }.instruction-form-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; align-items: end; }.instruction-form-grid label { display: grid; gap: 5px; color: #667085; font-size: 12px; }.instruction-form-grid input,.instruction-form-grid select { min-width: 0; padding: 9px 10px; border: 1px solid #cbdfe2; border-radius: 6px; background: #fff; }.instruction-file { grid-column: span 2; }
...
@media (max-width: 800px) { .instruction-form-grid { grid-template-columns: 1fr; }.instruction-file { grid-column: auto; }.instructions-heading { display: block; }.instructions-heading p { margin-top: 6px; } }
</style>
