...
const rows = ref<Instruction[]>([]),
    loading = ref(true),
    error = ref(""),
    saving = ref(false),
    editingId = ref<string | null>(null);
const advancedFilters = useFilterPresets("instructions");
...
const sections = [
    { key: "main", name: "Администрирование" },
    { key: "clients", name: "Клиенты" },
    { key: "goods", name: "Товары" },
    { key: "integration", name: "Интеграции" },
    { key: "fulfillment", name: "Фулфилмент" },
    { key: "maintenance", name: "Обслуживание" },
...
const advancedFields: FilterField[] = [
    ...
    { id: "shortname", label: "Код", type: "text" },
    { id: "section_key", label: "Раздел", type: "tuple" },
    {
        id: "content_type",
        label: "Формат",
        type: "tuple",
        format: (value) => types[String(value)] ?? String(value),
    },
    { id: "original_filename", label: "Имя файла", type: "text" },
...
async function load() {
    loading.value = true;
    try {
        const query = isAdminPage.value
            ? ""
            : section.value
              ? `?section=${encodeURIComponent(section.value)}`
              : "";
        rows.value = (await http(`/web/instructions${query}`)).data ?? [];
        if (props.instructionId) {
            selectedInstruction.value = (
                await http(`/web/instructions/${props.instructionId}`)
            ).data;
            if (selectedInstruction.value?.content_type === "markdown")
                selectedContent.value = await (
                    await fetch(selectedInstruction.value.content_url, {
...
async function save() {
    if (!editingId.value && !form.value.file) {
        error.value = "Выберите файл инструкции.";
        return;
    }
    saving.value = true;
    ...
    try {
        await http(
            editingId.value
                ? `/web/instructions/${editingId.value}`
                : "/web/instructions",
            "POST",
            data,
        );
...
async function remove(row: Instruction) {
    if (!window.confirm(`Удалить инструкцию «${row.name}»?`)) return;
    try {
        await http(`/web/instructions/${row.id}`, "DELETE");
        await load();
    } catch (e) {
        error.value =
            e instanceof HttpError
...
function download(row: Instruction) {
    window.location.href = row.download_url;
}
function openInstruction(row: Instruction) {
    window.location.href = section.value
        ? `/${section.value}/help/${row.id}`
        : `/instructions/${row.id}`;
}
function backToList() {
    window.location.href = section.value
...
function openImage(event: MouseEvent) {
    ...
    if (image instanceof HTMLImageElement)
        openedImage.value = image.currentSrc || image.src;
}
function closeImage() {
    openedImage.value = "";
}
...
<template>
    ...
    <section class="page-content instructions-page">
        <div class="content-breadcrumb">
            {{
                isAdminPage
                    ? "Администрирование › Инструкции"
                    : isViewing
                      ? `Инструкции › ${selectedInstruction?.name || "Загрузка"}`
        ...
        <div v-else class="instructions-heading">
            <h1>Инструкции</h1>
            <p class="muted">Инструкции по процессам текущего раздела</p>
        </div>
        <p v-if="error" class="notice error">{{ error }}</p>
        <div v-if="isViewing" class="instruction-reader">
            <div v-if="error" class="notice error">{{ error }}</div>
            <div v-else-if="!selectedInstruction" class="instruction-loading">
                Загрузка…
            </div>
            <template v-else>
                <div class="instruction-reader-toolbar">
                    <button
                        type="button"
                        class="instruction-back"
                        @click="backToList"
                    >
                        ← Инструкции
                    </button>
                    <a
                        class="instruction-download"
                        :href="selectedInstruction.download_url"
                ...
                <article
                    v-if="selectedInstruction.content_type === 'markdown'"
                    class="instruction-markdown"
                    v-html="renderedMarkdown"
                    @click="openImage"
                ></article>
                <iframe
                    v-else-if="selectedInstruction.content_type === 'html'"
                    class="instruction-frame"
                    :src="selectedInstruction.content_url"
                    sandbox=""
                ></iframe>
                <iframe
                    v-else-if="selectedInstruction.content_type === 'pdf'"
                    class="instruction-frame"
                    :src="selectedInstruction.content_url"
                ></iframe>
                <video
                    v-else-if="selectedInstruction.content_type === 'video'"
        ...
        <div v-else class="instructions-layout">
            <div class="instructions-list">
                <div class="page-heading">
                    <div>
                        ...
                        <p class="muted">
                            {{
                                isAdminPage
                                    ? "Справочник инструкций по процессам разделов"
                                    : "Инструкции по процессам текущего раздела"
                            }}
                        </p>
                    </div>
                    <FilterPresetButton
                ...
                <div class="table-scroll instructions-table-wrap">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td data-label="Название">
                                    ...
                                    ><small>{{ row.original_filename }}</small>
                                </td>
                ...
                <div class="instructions-mobile-cards">
                    <div v-if="loading" class="instructions-mobile-empty">
                        Загрузка…
                    </div>
                    <div
                        v-else-if="!filteredRows.length"
                        class="instructions-mobile-empty"
                    >
                        Инструкции не найдены
                    </div>
                    <article
                        ...
                        @keydown.enter="openInstruction(row)"
                    >
                        <span class="instructions-mobile-card__id">{{
                            row.id
                        }}</span>
                        <strong class="instructions-mobile-card__name">{{
                            row.name
                        }}</strong>
                        <div
                            v-if="isAdminPage"
                            class="instructions-mobile-card__actions"
                        >
                            <template>
                                <button
                                    type="button"
                                    aria-label="Изменить инструкцию"
                                    title="Изменить"
                                    @click.stop="edit(row)"
                                >
                                    ✎
            ...
            <aside
                ...
                aria-label="Создание и редактирование инструкции"
            >
                <h1>
                    {{
                        editingId
                ...
                <div class="instruction-form-grid">
                    ...
                    ><label
                        >Краткое имя<input
                            ...
                            type="text" /></label
                    ><label
                        >Раздел<select v-model="form.section_key">
                            <option
                    ...
                    ><label
                        >Порядок<input
                            ...
                            type="number"
                            min="0" /></label
                    ><label class="instruction-file"
                        >Файл<FileDropzone
                            v-model="form.file"
                            accept=".pdf,.md,.markdown,.html,.htm,.mp4"
                            hint="PDF, Markdown, HTML или видео"
                    /></label>
                    <div class="instruction-form-actions">
                        <button
                            class="primary"
                            type="button"
                            :disabled="saving"
        ...
        >
            ...
            <img
                :src="openedImage"
                alt="Полноразмерная иллюстрация инструкции"
                @click.stop
            />
        </div>
    </section>
...
.instruction-markdown {
    ...
    border-radius: 3px;
    background: #fffdf8;
    box-shadow: 0 10px 28px rgb(35 67 48 / 8%);
    color: #253237;
    font:
        16px/1.8 Manrope,
        sans-serif;
}
.instruction-markdown :deep(h1) {
    margin: 0 0 22px;
    color: #172126;
    font-size: 32px;
    line-height: 1.2;
}
.instruction-markdown :deep(h2) {
    margin: 30px 0 10px;
    padding-bottom: 6px;
...
.instruction-markdown :deep(h3) {
    margin: 24px 0 8px;
    color: #26547c;
    font-size: 17px;
}
.instruction-markdown :deep(h2:first-child),
.instruction-markdown :deep(h3:first-child) {
    margin-top: 0;
}
.instruction-markdown :deep(p) {
    margin: 0 0 17px;
    text-wrap: pretty;
}
...
.instruction-markdown :deep(ol),
.instruction-markdown :deep(ul) {
    margin: 10px 0 20px;
    padding: 14px 22px 14px 42px;
    border-left: 3px solid #a8d4a9;
    background: #f4faf5;
}
.instruction-markdown :deep(li) {
    margin: 5px 0;
    padding-left: 4px;
}
.instruction-markdown :deep(code) {
    padding: 2px 6px;
    border-radius: 4px;
    background: #e8f3eb;
    color: #176b2a;
...
.instruction-markdown :deep(img) {
    ...
    border-radius: 8px;
    box-shadow: 0 5px 14px rgb(35 67 48 / 10%);
    object-fit: cover;
    object-position: top;
    cursor: zoom-in;
}
.instruction-markdown :deep(img + img) {
    clear: right;
}
.instruction-markdown :deep(a) {
    color: #2274a5;
...
@media (max-width: 900px) {
    ...
    .instructions-mobile-card:not(:has(.instructions-mobile-card__actions)) {
        grid-template-columns: max-content minmax(0, 1fr);
    }
    .instructions-mobile-card:focus-visible {
        outline: 2px solid #2274a5;
        outline-offset: 2px;
    }
    .instructions-mobile-card__id {
        padding: 10px;
        color: #71828a;
        font-size: 11px;
    }
    .instructions-mobile-card__name {
        min-width: 0;
        padding: 10px 4px;
        overflow: hidden;
        color: #0c1821;
    ...
    .instructions-mobile-card__actions {
        display: flex;
        align-items: center;
        gap: 4px;
        padding: 6px 8px;
    ...
    .instructions-mobile-card__actions button {
        display: inline-grid;
        place-items: center;
        width: 32px;
        height: 32px;
    ...
    .instructions-mobile-card__actions button:hover {
        background: #cdebd7;
    }
    .instructions-mobile-empty {
        padding: 28px 16px;
        border: 1px solid #dcecef;
        border-radius: 10px;
        background: #fff;
