...
final class WorktimeService
{
    ...
    private function daily($events, array $days): array
    {
        ...
        foreach ($events as $event) {
            ...
            elseif ($event->event_type === 'finish' && $open) { $result[$date]['worked_minutes'] += $open->diffInMinutes($at); $result[$date]['finished_at'] = $at->toIso8601String(); $open = null; $result[$date]['state'] = 'finished'; }
        }
        return $result;
    }
