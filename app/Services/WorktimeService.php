...
final class WorktimeService
{
    ...
    public function event(User $user, string $action): array
    {
        return DB::transaction(function () use ($user, $action): array {
            ...
            $type = match ($action) {
                ...
                'finish' => in_array($state, ['working', 'paused'], true) ? 'finish' : null,
                default => null,
            };
            if ($type === null) {
    ...
    private function daily($events, array $days): array
    {
        ...
        foreach ($events as $event) {
            ...
            elseif ($event->event_type === 'finish' && $open) { if ($pause) { $result[$date]['pause_minutes'] += $pause->diffInMinutes($at); $pause = null; } $result[$date]['worked_minutes'] += max(0, $open->diffInMinutes($at) - $result[$date]['pause_minutes']); $result[$date]['finished_at'] = $at->toIso8601String(); $open = null; $result[$date]['state'] = 'finished'; }
        }
        return $result;
    }
