...
final class ImportTswmsCommand extends Command
{
        ...
        if ($clientConfig === []) {
            foreach ($options as $key => $value) {
                if (preg_match('/^client(?:_|-)?(\\d+)$/i', (string) $key, $match)) {
                    ...
                    if ($this->sourceClientId < 1) $this->sourceClientId = (int) $match[1];
                    break;
                }
            }
        }
        if ($billingConfig === [] || $clientConfig === []) throw new \RuntimeException('В options проекта должны быть блоки billing и client.');
    ...
    private function mysqlConfig(array $config): array
    {
        ...
        if ($ssl) {
            ...
            if (! is_file((string) $ca)) throw new \RuntimeException('SSL включён, но CA-сертификат не найден: '.$ca);
    ...
    private function sourceClientIdFromConfig(array $config): ?int
    {
        foreach ([$config['host'] ?? '', $config['database'] ?? '', $config['username'] ?? '', $config['password'] ?? ''] as $value) {
            if (preg_match('/clients\[(\d+)\]/', (string) $value, $match)) return (int) $match[1];
            if (preg_match('/tswms[_-]u(\d+)/i', (string) $value, $match)) return (int) $match[1];
        }
        return null;
    }
    ...
    {
        if (! $this->source->getSchemaBuilder()->hasTable('tswms-goods')) return;
        $labels = [];
        if ($this->source->getSchemaBuilder()->hasTable('tswms-goods-external-labels')) {
            foreach ($this->source->table('tswms-goods-external-labels')->get() as $label) {
                ...
                if ($goodId === '' || $value === '') continue;
                $type = strtolower((string) ($label->type ?? ''));
    ...
    {
        $kindMap = [];
        if ($this->source->getSchemaBuilder()->hasTable('warehouse_types')) foreach ($this->source->table('warehouse_types')->get() as $type) {
        ...
        $warehouseMap = [];
        if ($this->source->getSchemaBuilder()->hasTable('warehouses')) foreach ($this->source->table('warehouses')->get() as $warehouse) {
        ...
        if (! $this->source->getSchemaBuilder()->hasTable('tswms-places')) return;
        ...
        foreach($this->source->table('tswms-places')->get() as $row){$sourceWarehouse=(string)($row->warehouse_id??'');$placeType=strtolower((string)($row->type??''));$storageKey=str_contains($placeType,'transit')||str_contains($placeType,'транзит')?'transit':(str_contains($placeType,'box')||str_contains($placeType,'короб')?'box':'cell');$storageId=$storageMap[$storageKey];$this->upsert('wms.cells',['warehouse_id'=>$warehouseMap[$sourceWarehouse]??$warehouseId,'zone_id'=>$zoneId,'name'=>(string)($row->name??'Ячейка #'.$row->id),'shortname'=>'tswms_cell_'.$row->id,'row'=>(int)($row->row??0),'level'=>(int)($row->level??0),'number'=>(int)($row->number??$row->id),'priority'=>(int)($row->priority??0),'type_storage_id'=>$storageId?:null,'status'=>(int)($row->enabled??1),'tenant_id'=>$this->tenant,'created_at'=>now(),'updated_at'=>now()],'tswms-places',(string)$row->id,'App\\Models\\Cell');}
    }
    ...
    private function importAccounts(): void { $rows=[]; if($this->billing && $this->billing->getSchemaBuilder()->hasTable('legasy_integrations_registry')) $rows=array_merge($rows,array_map(fn($r)=>[$r,'legasy_integrations_registry'], $this->billing->table('legasy_integrations_registry')->where('client_id',$this->sourceClientId)->where('is_deleted',0)->get()->all())); if($this->source->getSchemaBuilder()->hasTable('tswms-integrations')) $rows=array_merge($rows,array_map(fn($r)=>[$r,'tswms-integrations'], $this->source->table('tswms-integrations')->get()->all())); foreach($rows as [$r,$table]){$partner=(string)($r->{'partner-id'}??$r->partner_id??'');$client=$this->mapped('tswms-partners',$partner,'App\\Models\\Client');if(!$client)continue;$type=(string)($r->type??'integration');$name=(string)($r->name??$type);$src=['source_system'=>'tswms','source_table'=>$table,'source_id'=>(string)$r->id,'source_fields'=>(array)$r,'credentials'=>['key1'=>$r->key1??null,'key2'=>$r->key2??null,'key3'=>$r->key3??null]];$serviceId=$this->mapped($table,(string)$r->id,'App\\Models\\ClientService');if(!$serviceId){$this->upsert('clients.services',['name'=>$name,'shortname'=>$this->latinShortname('service_'.$type.'_'.$r->id),'status'=>(int)($r->active??$r->is_active??1),'client_id'=>(int)$client,'tenant_id'=>$this->tenant,'created_at'=>now(),'updated_at'=>now()],$table,(string)$r->id,'App\\Models\\ClientService');$serviceId=$this->mapped($table,(string)$r->id,'App\\Models\\ClientService');}$data=['name'=>$name,'shortname'=>$this->latinShortname($type.'_'.$r->id),'host'=>$r->host??null,'login'=>$r->login??null,'pass'=>$r->pass??null,'token'=>$r->token??($r->key1??null),'descr'=>'Доступ TSWMS: '.$type,'status'=>(int)($r->active??$r->is_active??1),'src'=>json_encode($src,JSON_UNESCAPED_UNICODE),'client_id'=>(int)$client,'service_id'=>$serviceId ? (int)$serviceId : null,'tenant_id'=>$this->tenant,'created_at'=>now(),'updated_at'=>now()];$this->upsert('clients.accounts',$data,$table,(string)$r->id,'App\\Models\\ClientAccount');} }
    private function importWebhooks(): void { $rows=[]; if($this->billing && $this->billing->getSchemaBuilder()->hasTable('legasy_integrations_registry')) $rows=array_merge($rows,array_map(fn($r)=>[$r,'legasy_integrations_registry'], $this->billing->table('legasy_integrations_registry')->where('client_id',$this->sourceClientId)->where('is_deleted',0)->get()->all())); if($this->source->getSchemaBuilder()->hasTable('tswms-integrations')) $rows=array_merge($rows,array_map(fn($r)=>[$r,'tswms-integrations'], $this->source->table('tswms-integrations')->get()->all())); foreach($rows as [$r,$table]){$type=strtolower((string)($r->type??'integration'));$sourceId=(string)$r->id;$account=$this->mapped($table,$sourceId,'App\\Models\\ClientAccount');if(!$account)$account=$this->mapped('tswms-integrations',$sourceId,'App\\Models\\ClientAccount');$service=$this->localIntegrationService($type);$incoming=in_array(true,[(bool)($r->goods_active??$r->{'goods-active'}??false),(bool)($r->fbs_active??$r->{'fbs-active'}??false)],true);$params=['account_id'=>$account? (int)$account:null,'source'=>['table'=>$table,'id'=>$sourceId,'type'=>$type],'warehouse_id'=>$r->warehouse_id??null,'settings'=>$r->config_json??$r->{'config-json'}??null,'source_fields'=>(array)$r];$data=['name'=>(string)($r->name??$type),'shortname'=>$this->latinShortname($type.'_'.$sourceId),'service_id'=>$service,'type_hook_id'=>2,'rules_id'=>null,'params'=>json_encode($params,JSON_UNESCAPED_UNICODE),'status'=>(int)($r->active??$r->is_active??1),'cnt'=>0,'dat_last_run'=>$r->last_check_at??$r->{'last-check'}??null,'tenant_id'=>$this->tenant,'created_at'=>now(),'updated_at'=>now()];$this->upsert('integration.webhooks',$data,$table,$sourceId,'App\\Models\\IntegrationWebhook');} }
    private function localIntegrationService(string $type): ?int { $name=str_contains($type,'ozon')?'Ozon':(str_contains($type,'wb')||str_contains($type,'wildberries')?'Wildberries':(str_contains($type,'yandex')?'Yandex Market':$type));$id=DB::table('integration.services')->whereRaw('lower(name)=lower(?)',[$name])->value('id');if($id)return (int)$id;return (int)DB::table('integration.services')->insertGetId(['name'=>$name,'shortname'=>$this->latinShortname($name),'status'=>1,'tenant_id'=>$this->tenant,'created_at'=>now(),'updated_at'=>now()]); }
    private function latinShortname(string $value): string { $value=strtolower(trim($value));if(function_exists('transliterator_transliterate'))$value=transliterator_transliterate('Any-Latin; Latin-ASCII',$value)?:$value;$value=preg_replace('/[^a-z0-9]+/','_', $value)?:'integration';return trim($value,'_'); }
    ...
    private function importTaskStages(): void { if (! $this->source->getSchemaBuilder()->hasTable('tswms-tasks-statuses')) return; foreach($this->source->table('tswms-tasks-statuses')->get() as $r){$this->upsert('wms.task_stages',['name'=>(string)($r->name??'Этап #'.$r->id),'shortname'=>'tswms_stage_'.$r->id,'status'=>(int)($r->active??1),'icon'=>$r->icon??null,'tenant_id'=>$this->tenant,'src'=>json_encode(['source_system'=>'tswms','source_id'=>(string)$r->id,'source_fields'=>(array)$r],JSON_UNESCAPED_UNICODE),'created_at'=>now(),'updated_at'=>now()],'tswms-tasks-statuses',(string)$r->id,'App\\Models\\TaskStage');} }
    private function importImportedUsers(): void { $table=$this->source->getSchemaBuilder()->hasTable('users')?'users':($this->source->getSchemaBuilder()->hasTable('tswms-users')?'tswms-users':null); if(!$table)return; foreach($this->source->table($table)->get() as $r){$first=(string)($r->first_name??$r->firstname??'');$last=(string)($r->last_name??$r->lastname??'');$name=trim((string)($r->name??trim($last.' '.$first.' '.($r->patronymic??$r->middlename??''))));$this->upsert('wms.users',['code'=>(string)$r->id,'name'=>$name?:'Пользователь #'.$r->id,'shortname'=>'tswms_user_'.$r->id,'first_name'=>$first,'last_name'=>$last,'patronymic'=>$r->patronymic??$r->middlename??null,'email'=>$r->email??null,'phone'=>$r->phone??null,'login'=>$r->login??$r->username??null,'status'=>(int)($r->active??$r->is_active??1),'tenant_id'=>$this->tenant,'src'=>json_encode(['source_system'=>'tswms','source_id'=>(string)$r->id,'source_fields'=>(array)$r],JSON_UNESCAPED_UNICODE),'created_at'=>now(),'updated_at'=>now()],'users',(string)$r->id,'App\\Models\\ImportedUser');} }
    private function importTasks(): void { if(!$this->source->getSchemaBuilder()->hasTable('tswms-tasks'))return; $typeNames=$this->sourceNameMap(['tswms-tasks-types'],'name'); $priorityNames=$this->sourceNameMap(['tswms-tasks-priority'],'priority'); foreach($this->source->table('tswms-tasks')->get() as $r){$id=(string)$r->id;$client=$this->mapped('tswms-partners',(string)($r->{'partner-id'}??$r->partner_id??''),'App\\Models\\Client');$stage=$this->mapped('tswms-tasks-statuses',(string)($r->{'status-id'}??$r->status_id??''),'App\\Models\\TaskStage');$typeValue=$this->sourceValue($r,['task-type','task_type','type_id']);$typeName=is_numeric($typeValue)?($typeNames[(string)$typeValue]??null):$typeValue;$type= is_numeric($typeValue) ? ([1=>1,2=>5,3=>8,4=>9][(int)$typeValue]??$this->findLocalNameOnly('wms.task_types',$typeName)) : $this->findLocalNameOnly('wms.task_types',$typeName);$priorityValue=$this->sourceValue($r,['priority-id','priority_id']);$priorityName=is_numeric($priorityValue)?($priorityNames[(string)$priorityValue]??null):$priorityValue;$priority=$this->findLocalNameOnly('wms.priorities',$priorityName);if($priority===null&&is_numeric($priorityValue)){$priority=[1=>3,2=>4,3=>5,4=>8][(int)$priorityValue]??null;}$warehouse=$this->mapped('warehouses',(string)($r->warehouse_id??$r->{'warehouse-id'}??''),'App\\Models\\Warehouse');$creator=$this->mappedUser($r->{'creator-id'}??$r->creator_id??null);$responsible=$this->mappedUser($r->{'charged-user-id'}??$r->user_id??null);$data=['code'=>$id,'name'=>(string)($r->{'task-name'}??$r->name??'Задача #'.$id),'shortname'=>'tswms_task_'.$id,'client_id'=>$client,'task_type_id'=>$type,'task_stage_id'=>$stage,'status_id'=>1,'planned_at'=>$r->{'date-plan'}??$r->planned_at??null,'started_at'=>$r->started_at??null,'completed_at'=>$r->{'date-fact'}??$r->completed_at??null,'comment'=>$r->{'client-comment'}??$r->comment??null,'internal_comment'=>$r->{'internal-comment'}??$r->internal_comment??null,'priority_id'=>$priority,'order_id'=>$r->{'created-order-id'}??$r->order_id??null,'warehouse_id'=>$warehouse,'user_id'=>$responsible,'created_by_user_id'=>$creator,'charged_at'=>$r->{'charged-date'}??$r->charged_at??null,'fact_count'=>(int)($r->fact_count??0),'status'=>(int)($r->active??1),'tenant_id'=>$this->tenant,'src'=>json_encode(['source_system'=>'tswms','source_id'=>$id,'source_fields'=>(array)$r],JSON_UNESCAPED_UNICODE),'created_at'=>now(),'updated_at'=>now()];$this->upsert('wms.tasks',$data,'tswms-tasks',$id,'App\\Models\\Task');} }
    private function importTaskGoods(): void { if(!$this->source->getSchemaBuilder()->hasTable('tswms-tasks-goods'))return; foreach($this->source->table('tswms-tasks-goods')->get() as $r){$task=$this->mapped('tswms-tasks',(string)($r->{'task-id'}??$r->task_id??''),'App\\Models\\Task');if(!$task){continue;}$good=$this->mapped('tswms-goods',(string)($r->{'good-id'}??$r->good_id??''),'App\\Models\\Good');$id=(string)$r->id;$this->upsert('wms.task_goods',['task_id'=>$task,'good_id'=>$good,'shipment_box_number'=>$r->{'shipment-box-number'}??null,'count_plan'=>$r->{'count-plan'}??$r->count_plan??null,'count_fact'=>$r->count_fact??$r->{'count-fact'}??null,'good_comment'=>$r->{'good-comment'}??null,'good_current_barcode'=>$r->{'good-current-barcode'}??null,'unit_price'=>$r->{'unit-price'}??null,'unit_price_currency_id'=>$r->{'unit-price-currency-id'}??null,'tenant_id'=>$this->tenant,'src'=>json_encode(['source_system'=>'tswms','source_id'=>$id,'source_fields'=>(array)$r],JSON_UNESCAPED_UNICODE),'created_at'=>now(),'updated_at'=>now()],'tswms-tasks-goods',$id,'App\\Models\\TaskGood');} }
    private function sourceValue(object $row,array $keys): mixed { foreach($keys as $key){$v=$row->{$key}??null;if($v!==null&&$v!=='')return $v;}return null; }
    private function sourceNameMap(array $tables,string $field='name'): array { foreach($tables as $table){if(!$this->source->getSchemaBuilder()->hasTable($table))continue;$map=[];foreach($this->source->table($table)->get() as $r){$map[(string)$r->id]=(string)($r->{$field}??$r->name??$r->title??'');}return $map;}return []; }
    private function findLocalNameOnly(string $table,mixed $value): ?int { if($value===null||$value==='')return null; $value=strtolower(str_replace('ё','е',trim((string)$value))); $aliases=['прием поставки'=>'приемка','приём поставки'=>'приемка','отгрузка товара'=>'отгрузка','обработка товара'=>'обработка','прочие задачи'=>'прочие задачи','низкий'=>'очень низкий','стандартный'=>'средний','высокий'=>'высокий','супер срочный'=>'критический']; $value=$aliases[$value]??$value; foreach(DB::table($table)->whereNull('deleted_at')->get(['id','name']) as $row){$name=strtolower(str_replace('ё','е',trim((string)$row->name))); if($name===$value||str_contains($name,$value)||str_contains($value,$name))return (int)$row->id;} return null; }
    private function findLocalByName(string $table,mixed $value): ?int { if($value===null)return null; $q=DB::table($table)->whereNull('deleted_at'); if(is_numeric($value)&&($id=$q->where('id',(int)$value)->value('id')))return (int)$id; return $q->whereRaw('lower(name)=lower(?)',[(string)$value])->value('id'); }
    private function mappedUser(mixed $id): ?int { $value=$id===null?null:$this->mapped('users',(string)$id,'App\\Models\\ImportedUser'); return $value===null?null:(int)$value; }
    ...
    private function upsert(string $table,array $data,string $sourceTable,string $sourceId,string $entity,string $lookup=''): void
    {
        ...
        if ($mapping && (string) $mapping->source_hash === $hash) return;
        $target = $mapping?->target_id;
        if ($target) { DB::table($table)->where('id',$target)->update($data); $this->updated++; }
        else { $target=(string)DB::table($table)->insertGetId($data); $this->created++; }
    ...
    private function remember(string $table,string $id,string $entity,string $target,$row): void { DB::table('wms.tswms_import_mappings')->updateOrInsert(['source_system'=>'tswms','source_client_id'=>$this->sourceClientId,'source_table'=>$table,'source_id'=>$id,'target_entity'=>$entity],['target_id'=>$target,'source_database'=>$this->sourceDatabase,'source_hash'=>$this->sourceHash($row),'updated_at'=>now(),'created_at'=>now()]); }
    private function sourceHash(object $row): string { $data=(array)$row; unset($data['created_at'],$data['updated_at']); return hash('sha256','v9|'.json_encode($data,JSON_UNESCAPED_UNICODE)); }
}
