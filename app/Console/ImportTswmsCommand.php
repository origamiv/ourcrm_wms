...
final class ImportTswmsCommand extends Command
{
    ...
    private function importAccounts(): void
    {
        ...
        if ($this->billing && $this->billing->getSchemaBuilder()->hasTable('legasy_integrations_registry')) {
            ...
            if (! $serviceId) {
                $this->upsert('clients.services', ['name' => $name, 'shortname' => $this->latinShortname('service_'.$type.'_'.$r->id), 'status' => (int) ($r->active ?? $r->is_active ?? 1), 'client_id' => (int) $client, 'tenant_id' => $this->tenant, 'created_at' => now(), 'updated_at' => now()], $table, (string) $r->id, 'App\\Models\\ClientService');
                $serviceId = $this->mapped($table, (string) $r->id, 'App\\Models\\ClientService');
