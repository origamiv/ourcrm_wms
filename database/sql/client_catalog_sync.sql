...
CREATE TRIGGER wms_client_services_change AFTER INSERT OR UPDATE OR DELETE ON clients.services FOR EACH ROW EXECUTE FUNCTION wms.capture_entity_change('App\Models\ClientService', '["name","shortname","status","client_id","tenant_id","created_at","updated_at","deleted_at"]');
...
BEGIN
    FOR item IN SELECT id::text AS id, tenant_id, entity, data FROM (
        SELECT id, tenant_id, 'App\Models\ClientService' AS entity, jsonb_build_object('id', id::text, 'name', name, 'shortname', shortname, 'status', status, 'client_id', client_id, 'tenant_id', tenant_id, 'created_at', created_at, 'updated_at', updated_at, 'deleted_at', deleted_at) AS data FROM clients.services
        UNION ALL
        SELECT id, tenant_id, 'App\Models\ClientAccount', jsonb_build_object('id', id::text, 'name', name, 'shortname', shortname, 'host', host, 'login', login, 'pass', pass, 'token', token, 'descr', descr, 'status', status, 'group_id', group_id, 'server_id', server_id, 'src', src, 'client_id', client_id, 'tenant_id', tenant_id, 'created_at', created_at, 'updated_at', updated_at, 'deleted_at', deleted_at) FROM clients.accounts
