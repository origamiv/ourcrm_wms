...
Миграция 000025 заполняет краткие названия служб доставки и создаёт восемь общих маркетплейсов: Wildberries (wb), OZON (oz), СберМегаМаркет (smm), ЛеруаМерлен (lm), Yandex Market (ym), AliExpress (ali), МВидео (mvideo) и Сайт (site). FBS/FBO/FBP-службы привязаны к соответствующим площадкам; самостоятельные службы доставки остаются без связи. Все краткие названия используют только строчные латинские буквы, цифры, точку или подчёркивание.

Миграция 000026 приводит к нижнему регистру краткие названия уже существующих записей. Общее правило справочников: `shortname` — необязательное короткое имя из строчных латинских букв, цифр и `_`, без дефисов; активные значения уникальны в пределах своей таблицы. Это правило применяется к новым и изменяемым записям, а таблицы маркетплейсов и служб доставки дополнительно защищены уникальными индексами.
