...
В разделе «Клиенты → Интеграции» (`/clients/integrations`) используется та же таблица вебхуков и стандартные действия. Переход из строки клиента добавляет `client_id` в адрес и показывает только его записи; клиент подставляется при создании и проверяется при прямом открытии карточки. Поле `client_id` включено в синхронизируемые данные списка, но параметры доступа по-прежнему загружаются только для карточки.
