...
test("fulfillment catalogs create edit and cache marketplace delivery links", async ({ page, context }) => {
    await login(page);
    await page.locator(".sidebar").getByRole("link", { name: "Фулфилмент", exact: true }).click();
    await expect(page).toHaveURL("/fulfillment/marketplaces");
    await page.getByRole("button", { name: "Справочники", exact: false }).click();
    await expect(page.getByRole("navigation", { name: "Справочники", exact: true }).getByRole("link", { name: "Маркетплейсы", exact: true })).toBeVisible();
    await page.getByRole("link", { name: "Маркетплейсы", exact: true }).click();
    await page.getByRole("button", { name: "Добавить запись", exact: true }).click();
    await page.getByLabel("Название *", { exact: true }).fill("Маркет браузера");
    await page.getByLabel("Иконка", { exact: true }).fill("market");
    await page.getByRole("button", { name: "Создать запись", exact: true }).click();
    await expect(page.getByText("Изменения сохранены", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "Закрыть карточку" }).click();
    await page.getByRole("button", { name: "Справочники", exact: false }).click();
    await page.getByRole("link", { name: "Службы доставки", exact: true }).click();
    await expect(page).toHaveURL("/fulfillment/delivery_services");
    await page.getByRole("button", { name: "Добавить запись", exact: true }).click();
    await page.getByLabel("Название *", { exact: true }).fill("Доставка браузера");
    await page.getByLabel("Маркетплейс", { exact: true }).selectOption({ label: "Маркет браузера" });
    await page.getByLabel("Цвет", { exact: true }).fill("#123456");
    await page.getByLabel("Разрешено редактирование заказа", { exact: true }).selectOption("1");
    await page.getByLabel("Префикс", { exact: true }).fill("DEL");
    await page.getByLabel("Папка", { exact: true }).fill("deliveries");
    await page.getByRole("button", { name: "Создать запись", exact: true }).click();
    await expect(page.getByText("Изменения сохранены", { exact: true })).toBeVisible();
    await page.getByLabel("Префикс", { exact: true }).fill("DEL2");
    await page.getByRole("button", { name: "Сохранить изменения", exact: true }).click();
    await expect(page.getByText("Изменения сохранены", { exact: true })).toBeVisible();
    await page.reload();
    await expect(page.getByLabel("Префикс", { exact: true })).toHaveValue("DEL2");
    await expect(page.getByLabel("Маркетплейс", { exact: true }).locator("option:checked")).toHaveText("Маркет браузера");
    await page.getByRole("button", { name: "Закрыть карточку" }).click();
    await context.setOffline(true);
    await page.getByRole("button", { name: "Справочники", exact: false }).click();
    await page.getByRole("link", { name: "Маркетплейсы", exact: true }).click();
    await expect(page.getByRole("button", { name: "Маркет браузера", exact: true })).toBeVisible();
    await expect(page.getByRole("button", { name: "Добавить запись", exact: true })).toBeDisabled();
    await context.setOffline(false);
});
